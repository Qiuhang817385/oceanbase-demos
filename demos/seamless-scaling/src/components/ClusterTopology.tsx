...
export function ClusterTopology({ config, scalingState, scalingDirection, zones, currentQPS = 0, theme }: ClusterTopologyProps) {
  ...
  const hasOldZones = zones.some(z => z.isOld); // 检查是否有旧Zone（表示已切主）
  ...
  return (
    <div className={`rounded-lg p-4 h-full flex flex-col border ${theme === 'dark' ? 'bg-slate-900 border-slate-800' : 'bg-white border-gray-200'}`}>
      ...
      <div className="space-y-4">
        ...
        <div className={`rounded-lg p-5 border ${theme === 'dark' ? 'border-slate-700' : 'border-gray-300'}`}>
          {/* Zones Grid */}
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`border-2 rounded-lg p-3 ${
                      ...
                      {/* Badge for new/old zones */}
                      {style.badge && (
                        <motion.div
                          initial={{ opacity: 0, scale: 0.8 }}
                          animate={{ opacity: 1, scale: 1 }}
                          className={`absolute -top-2 -left-2 px-2 py-0.5 ${style.badge} text-white text-xs rounded-full shadow-lg`}
                        >
                          {style.badgeText}
                        </motion.div>
                      )}
