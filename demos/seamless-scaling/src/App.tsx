...
export type ScalingState = 'idle' | 'scaling-out' | 'scaling-out-migrating' | 'scaling-in' | 'scaling-in-migrating' | 'switching-primary';
...
export default function App() {
  ...
  const [config, setConfig] = useState<ClusterConfig>({ zones: 2, serversPerZone: 2 });
  const [scalingState, setScalingState] = useState<ScalingState>('idle');
  const [scalingDirection, setScalingDirection] = useState<ScalingDirection>(null);
  ...
  // Add log entry
  const addLog = (message: string, type: LogEvent['type'] = 'info', isPrimarySwitchEvent = false) => {
    setLogs(prev => [
      {
        id: logIdRef.current++,
        timestamp: Date.now(),
  ...
  const handleScaleOut = async () => {
    ...
    addLog(`➕ 添加新的 Zone-${newId1} 和 Zone-${newId2} (各4台 OBServer)`, 'info');
    setScalingState('scaling-out');
    setZones(prev => [
      ...prev, 
      { id: newId1, name: `Zone-${newId1}`, isPrimary: false, observerCount: 4, isNew: true },
      { id: newId2, name: `Zone-${newId2}`, isPrimary: false, observerCount: 4, isNew: true }
    ]);
    ...
    addLog(`✅ 临时扩展至 4F1A (${oldZoneNames}, Zone-${newId1}[4台], Zone-${newId2}[4台])`, 'success');
    ...
    addLog(`   - 旧 Zone (${zones.filter(z => oldZoneIds.includes(z.id)).map(z => z.name).join(', ')}): 主可用区，各2台 OBServer，Leader + Follower`, 'info');
    addLog(`   - 新 Zone (Zone-${newId1}, Zone-${newId2}): 备区，各4台 OBServer，Follower`, 'info');
    ...
    // Step 2: 切换主可用区（瞬时操作）
    await new Promise(resolve => setTimeout(resolve, 2000));
    addLog(`🔀 切换主可用区：旧 Zone (${zones.filter(z => !z.isNew).map(z => z.name).join(', ')}) → 新 Zone (Zone-${newId1}, Zone-${newId2})`, 'info', true);
    setScalingState('switching-primary');
    switchingStartTimeRef.current = Date.now(); // 记录切主开始时间
    preSwitchValuesRef.current = { qps: currentValuesRef.current.qps, tps: currentValuesRef.current.tps }; // 记录切主前的指标值
    
    await new Promise(resolve => setTimeout(resolve, 3000));
    const oldZoneIdsToRemove = zones.filter(z => z.id !== newId1 && z.id !== newId2).map(z => z.id);
    ...
    addLog(`✅ 主可用区切换完成：Zone-${newId1}, Zone-${newId2} 现为主可用区`, 'success');
    ...
    addLog(`   - 新 Zone (Zone-${newId1}, Zone-${newId2}): 主可用区，各4台 OBServer，Leader + Follower`, 'info');
    addLog(`   - 旧 Zone (${zones.filter(z => oldZoneIdsToRemove.includes(z.id)).map(z => z.name).join(', ')}): 备区，各2台 OBServer，Follower`, 'info');
    ...
    await new Promise(resolve => setTimeout(resolve, 1500));
    const oldZoneNamesToDelete = zones.filter(z => oldZoneIdsToRemove.includes(z.id)).map(z => z.name).join(', ');
    addLog(`🗑️ 删除原 Zone (${oldZoneNamesToDelete}) 中...`, 'warning');
    setScalingState('scaling-out-migrating'); // 进入删除阶段
    setZones(prev => prev.map(z => 
      oldZoneIdsToRemove.includes(z.id) ? { ...z, isDeleting: true, isOld: false } : z
    ));
    ...
    addLog(`📌 新配置：2F1A (Zone-${newId1}[4台主可用区], Zone-${newId2}[4台])`, 'success');
  ...
  const handleScaleIn = async () => {
    ...
    // Step 1: 添加新的小规模 Zone
    await new Promise(resolve => setTimeout(resolve, 1500));
    addLog(`➕ 添加新的 Zone-${newId1} 和 Zone-${newId2} (各2台 OBServer)`, 'info');
    setScalingState('scaling-in');
    setZones(prev => [
      ...prev, 
      { id: newId1, name: `Zone-${newId1}`, isPrimary: false, observerCount: 2, isNew: true },
      { id: newId2, name: `Zone-${newId2}`, isPrimary: false, observerCount: 2, isNew: true }
    ]);
    ...
    addLog(`✅ 临时扩展至 4F1A (${oldZoneNames}, Zone-${newId1}[2台], Zone-${newId2}[2台])`, 'success');

    // 此时 4 个 Zone 同时存在，但还未切主：旧 Zone 为主区（Leader+Follower），新 Zone 为备区（Follower）
    await new Promise(resolve => setTimeout(resolve, 2000));
    const oldZoneIds = zones.map(z => z.id);
    addLog(`📊 当前状态：4个 Zone 共存（切主前）`, 'info');
    addLog(`   - 旧 Zone (${zones.filter(z => oldZoneIds.includes(z.id)).map(z => z.name).join(', ')}): 主区，各4台 OBServer，Leader + Follower`, 'info');
    addLog(`   - 新 Zone (Zone-${newId1}, Zone-${newId2}): 备区，各2台 OBServer，Follower`, 'info');
    ...
    await new Promise(resolve => setTimeout(resolve, 2000));
    addLog(`📊 当前状态：4个 Zone 共存（切主后）`, 'info');
    addLog(`   - 新 Zone (Zone-${newId1}, Zone-${newId2}): 主可用区，各2台 OBServer，Leader + Follower`, 'info');
    addLog(`   - 旧 Zone (${zones.filter(z => oldZoneIdsToRemove.includes(z.id)).map(z => z.name).join(', ')}): 备区，各4台 OBServer，Follower`, 'info');
    ...
    addLog(`📌 新配置：2F1A (Zone-${newId1}[2台主可用区], Zone-${newId2}[2台])`, 'success');
  };
  ...
  useEffect(() => {
    if (autoMode && justCompletedScaleOut && scenario === 'normal' && scalingState === 'idle') {
      const timer = setTimeout(() => {
      ...
      // 否则，如果还未扩容，说明是初始正常流量或缩容后的正常流量，需要提前扩容为下一轮大促做准备
        ...
        const timer = setTimeout(() => {
          if (cycleCountRef.current === 0) {
            ...
            addLog(`🔄 [自动模式] 第 ${cycleCountRef.current + 1} 轮大促演示开始，正常流量阶段，提前扩容为大促做准备`, 'info');
          }
          handleScaleOut();
        }, delay);
  ...
  }, [autoMode, scenario, isScaledOut, scalingState, isMetricsStable, isPaused]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      switch (scenario) {
        ...
        case 'warming-up':
          ...
          if (scalingState === 'idle' && !isScaledOut) {
            ...
            targetQPS = 15000;
            targetTPS = 12000;
          } else if (scalingState === 'scaling-out' || scalingState === 'scaling-out-migrating') {
            // 扩容中：继续上升到 22k
            targetQPS = 22000;
            targetTPS = 17500;
      ...
      if (scalingState === 'switching-primary' && switchingStartTimeRef.current !== null) {
        ...
        if (timeSinceSwitchStart < 2000) {
          ...
          if (timeSinceSwitchStart < 800) {
            ...
            const dropRatio = 1.0 - dropProgress * 0.10; // 从 100% 降到 90%
          ...
          // 0.8-2 秒：快速恢复到正常水平
            ...
            const recoveredRatio = 0.90 + recoveryProgress * 0.10; // 从 90% 恢复到 100%
            const currentTarget = currentQPS + (targetQPS - currentQPS) * smoothFactor;
            newQPS = preSwitchValuesRef.current.qps * recoveredRatio + (currentTarget - preSwitchValuesRef.current.qps) * recoveryProgress;
            newTPS = preSwitchValuesRef.current.tps * recoveredRatio + (currentTPS + (targetTPS - currentTPS) * smoothFactor - preSwitchValuesRef.current.tps) * recoveryProgress;
          }
        } else {
          // 切主完成，清除切主开始时间
      ...
      if (scalingState === 'scaling-out' || scalingState === 'scaling-in' || scalingState === 'scaling-out-migrating' || scalingState === 'scaling-in-migrating') {
      ...
      if (recentMetricsRef.current.length === 5) {
        ...
        if (range < 500) { // 如果波动范围小于 500 QPS，则认为流量平稳
  ...
  return (
    <div className={`min-h-screen p-4 ${theme === 'dark' ? 'bg-slate-950 text-slate-100' : 'bg-gray-50 text-gray-900'}`}>
      {/* Header */}
      <header className={`mb-4 border-b pb-3 ${theme === 'dark' ? 'border-slate-800' : 'border-gray-200'}`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Database className={`w-7 h-7 ${theme === 'dark' ? 'text-blue-400' : 'text-blue-600'}`} />
            <div>
              <h1 className={theme === 'dark' ? 'text-slate-100' : 'text-gray-900'}>OceanBase 平滑扩缩容演示</h1>
              <p className={`text-sm ${theme === 'dark' ? 'text-slate-500' : 'text-gray-500'}`}>电商大促场景 - 基于异构 Zone 的平滑扩缩容: 2F1A (2 台 OBServer) ⇄ 4F1A ⇄ 2F1A (4 台 OBServer)</p>
            </div>
          </div>
          ...
          <Button
            ...
            className={`h-9 px-3 ${theme === 'dark' ? 'bg-slate-800 hover:bg-slate-700 border border-slate-700' : 'bg-white hover:bg-gray-100 border border-gray-300 text-gray-900'}`}
      ...
      <div className="grid grid-cols-2 gap-4" style={{ height: 'calc(100vh - 200px)' }}>
        ...
        <div className="h-full">
          <ClusterTopology 
            ...
            scalingDirection={scalingDirection}
            zones={zones}
            currentQPS={metrics.length > 0 ? metrics[metrics.length - 1].qps : 0}
            theme={theme}
          />
        </div>
      </div>
